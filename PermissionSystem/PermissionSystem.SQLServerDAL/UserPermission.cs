...
namespace PermissionSystem.SQLServerDAL
{
	public partial class UserPermission:IUserPermission
	{
		...
		public bool Exists(long user_permission_id)
		{
			...
			strSql.Append("select count(1) from UserPermission");
			strSql.Append(" where user_permission_id=@user_permission_id");
			SqlParameter[] parameters = {
					new SqlParameter("@user_permission_id", SqlDbType.BigInt)
			};
			parameters[0].Value = user_permission_id;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into UserPermission(");
			strSql.Append("user_id,permission_id,permission_type_id)");
		...
		public bool Delete(long user_permission_id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from UserPermission ");
			strSql.Append(" where user_permission_id=@user_permission_id");
			SqlParameter[] parameters = {
					new SqlParameter("@user_permission_id", SqlDbType.BigInt)
			};
		...
		public bool DeleteList(string user_permission_idlist )
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from UserPermission ");
			strSql.Append(" where user_permission_id in ("+user_permission_idlist + ")  ");
		...
		public PermissionSystem.Model.UserPermission GetModel(long user_permission_id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 user_permission_id,user_id,permission_id,permission_type_id from UserPermission ");
			strSql.Append(" where user_permission_id=@user_permission_id");
			SqlParameter[] parameters = {
					new SqlParameter("@user_permission_id", SqlDbType.BigInt)
			};
			...
			PermissionSystem.Model.UserPermission model=new PermissionSystem.Model.UserPermission();
		...
		/// 得到一个对象实体
		/// </summary>
		public PermissionSystem.Model.UserPermission DataRowToModel(DataRow row)
		{
			PermissionSystem.Model.UserPermission model=new PermissionSystem.Model.UserPermission();
			if (row != null)
			{
				if(row["user_permission_id"]!=null && row["user_permission_id"].ToString()!="")
				{
					model.user_permission_id=long.Parse(row["user_permission_id"].ToString());
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select user_permission_id,user_id,permission_id,permission_type_id ");
			strSql.Append(" FROM UserPermission ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" user_permission_id,user_id,permission_id,permission_type_id ");
			strSql.Append(" FROM UserPermission ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			{
				strSql.Append("order by T.user_permission_id desc");
			}
			strSql.Append(")AS Row, T.*  from UserPermission T ");
